...
print(1)
# nltk.download("popular", quiet=False)
# nltk.download("universal_tagset", quiet=False)
...
class VisualizationDemo(object):
    ...
    def run_on_image_optimize(self, image):
        """
        Args:
        ...
        if "panoptic_seg" in predictions:
            ...
            panoptic_seg_np = panoptic_seg.to(self.cpu_device).numpy().astype(np.int32)
            original_image_np = image.copy()

            # ============== 核心：计算每个区域的RGB均值 ==============
            segment_colors = {}
            alpha = 0.7  # 透明度，越大颜色越深
            for seg_info in segments_info:
                ...
                # 计算当前区域像素的 BGR 均值
                b = np.mean(original_image_np[mask, 0])
                g = np.mean(original_image_np[mask, 1])
                r = np.mean(original_image_np[mask, 2])
                segment_colors[seg_id] = (int(b), int(g), int(r))
            ...
            for seg_id, color in segment_colors.items():
                mask = panoptic_seg_np == seg_id
                # 半透明混合
                output_img[mask] = cv2.addWeighted(output_img[mask], 1 - alpha, np.full_like(output_img[mask], color),
                                                   alpha, 0)

            # 返回最终图像
            from detectron2.utils.visualizer import VisImage
            vis_output = VisImage(output_img)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config_file",
        default="/media/data/songzengyu/Brain-Imager/code/ODISE-main/configs/Panoptic/odise_label_coco_50e.py",
        type=str,
    ...
    parser.add_argument(
        ...
        default="/media/data/songzengyu/Brain-Imager/code/ODISE-main/odise_label_coco_50e-b67d2efc.pth",
    )
    ...
    parser.add_argument(
        ...
        default=["/media/data/songzengyu/Brain-Imager/code/run_server_github/result/gt_all/*.jpg"],
    )
    parser.add_argument(
        "--output",
        help="A file or directory to save output visualizations. "
        "If not given, will show output in an OpenCV window.",
        default="/media/data/songzengyu/Brain-Imager/code/ODISE-main/brainImager/testData_result",
    )
    parser.add_argument(
        "--vocab",
    ...
    with ExitStack() as stack:
        ...
        if args.input:
            ...
            for path in tqdm.tqdm(args.input, disable=not args.output):
                ...
                predictions, visualized_output = demo.run_on_image_optimize(img)
                logger.info(
                    "{}: {} in {:.2f}s".format(
                        path,
