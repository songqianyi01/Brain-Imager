...
def get_huggingface_urls(commit='main',subj=1):
    ...
    train_url = base_url + commit + f"/webdataset_avg_split/self_train/train_subj0{subj}_" + "{0..17}.tar"
...
def get_dataloaders(
    ...
    if num_train is None:
        ...
        num_train = metadata['totals']['self_train']
    if num_val is None:
        metadata = json.load(open(meta_url))
...
laion_transform2=transforms.Compose([
    transforms.Resize((768),antialias=True),
...
def query_laion(text=None, emb=None, num=8, indice_name="laion5B-L-14", groundtruth=None, clip_extractor=None, device=None, verbose=False):
    ...
    if groundtruth is not None:
        if indice_name == "laion5B-L-14":
            groundtruth = transforms.Resize((512,512),antialias=True)(groundtruth)
        elif indice_name == "laion5B-H-14":
            groundtruth = transforms.Resize((768,768),antialias=True)(groundtruth)
...
def reconstruction_image_embeddings(
        image, voxel,
        clip_extractor,
        unet=None,
        vae=None,
...
def reconstruction_proj_embeddings(
        image, voxel,
        clip_extractor,
        unet=None,
        vae=None,
...
def reconstruction_integrity(
    ...
    if 1:
        input_embedding = input_embedding  # .repeat(recons_per_sample, 1, 1)
    ...
    if retrieve == False:
        ...
        sims = []
        for im in range(recons_per_sample):
            currecon = clip_extractor.embed_image(brain_recons[0,[im]].float()).to(proj_embedding.device).to(
                proj_embedding.dtype)
            currecon = nn.functional.normalize(currecon.view(len(currecon), -1), dim=-1)
            cursim = batchwise_cosine_similarity(v2c_reference_out, currecon)
            sims.append(cursim.item())
        if verbose: print(sims)
...
def reconstruction_integrity_noise_assign(
        clip_extractor,
        unet1=None,
        unet2=None,
        vae=None,
    ...
    if 1:
        ...
        if do_classifier_free_guidance:
            input_embedding1 = torch.cat([torch.zeros_like(input_embedding), input_embedding]).to(device).to(unet1.dtype)
            input_embedding2 = torch.cat([torch.zeros_like(input_embedding), torch.zeros_like(input_embedding)]).to(device).to(unet2.dtype)
            prompt_embeds1 = torch.cat([torch.zeros_like(prompt_embeds), prompt_embeds]).to(device).to(unet2.dtype)
            prompt_embeds2 = torch.cat([torch.zeros_like(prompt_embeds), torch.zeros_like(prompt_embeds)]).to(device).to(unet1.dtype)

        # dual_prompt_embeddings
        if not img_variations:
            image_embedding = torch.cat([prompt_embeds2, input_embedding1], dim=1)
            text_embedding = torch.cat([prompt_embeds1, input_embedding2], dim=1)

        # 4. Prepare timesteps
        noise_scheduler.set_timesteps(num_inference_steps=num_inference_steps, device=device)

        # 5b. Prepare latent variables
        batch_size = image_embedding.shape[0] // 2  # divide by 2 bc we doubled it for classifier-free guidance
        ...
        if img_lowlevel is not None:  # use img_lowlevel for img2img initialization
            ...
            noise = torch.randn([recons_per_sample, 4, 64, 64], device=device,
                                generator=generator, dtype=image_embedding.dtype)
            init_latents = noise_scheduler.add_noise(init_latents, noise, latent_timestep)
            latents = init_latents
        else:
            timesteps = noise_scheduler.timesteps
        ...
        for i, t in enumerate(timesteps):
            ...
            noise_pred1 = unet1(latent_model_input, t, encoder_hidden_states=image_embedding).sample
            noise_pred2 = unet2(latent_model_input, t, encoder_hidden_states=text_embedding).sample
            lameta = 1/(1+np.exp(-1.5*(i-3)))
            noise_pred = lameta * noise_pred1+(1-lameta) * noise_pred2
...
def reconstruction_integrity_dynamic_fusion(
        clip_extractor,
        unet=None,
        vae=None,
        noise_scheduler=None,
    ...
    if 1:
        ...
        for i, t in enumerate(timesteps):
            # expand the latents if we are doing classifier free guidance
            lameta = 1/(1+np.exp(-2*(i-3)))
            for name, module in unet.named_modules():
                if isinstance(module, DualTransformer2DModel):
                    module.mix_ratio = (1-lameta)
            latent_model_input = torch.cat([latents] * 2) if do_classifier_free_guidance else latents
            latent_model_input = noise_scheduler.scale_model_input(latent_model_input, t)
...
def reconstruction_integrity_only_text(
        image, voxel,
        clip_extractor,
        unet=None,
        vae=None,
    ...
    if 1:
        ...
        if do_classifier_free_guidance:
            input_embedding = torch.cat([torch.zeros_like(input_embedding), torch.zeros_like(input_embedding)]).to(device).to(unet.dtype)
            prompt_embeds = torch.cat([torch.zeros_like(prompt_embeds), prompt_embeds]).to(device).to(unet.dtype)
...
def reconstruction_integrity_trial(
        clip_extractor,
        unet=None,
        vae=None,
        noise_scheduler=None,
...
def reconstruction_integrity_16x16(
        clip_extractor,
        recons_per_sample=None,
        n_samples_save=None,
        proj_embedding=None,
        brain_recons = None
):
    ...
    if 1:
        ...
        sims = []
        for im in range(recons_per_sample):
            currecon = clip_extractor.embed_image(brain_recons[im].float()).to(proj_embedding.device).to(
                proj_embedding.dtype)
            currecon = nn.functional.normalize(currecon.view(len(currecon), -1), dim=-1)
            cursim = batchwise_cosine_similarity(v2c_reference_out, currecon)
            sims.append(cursim.item())
